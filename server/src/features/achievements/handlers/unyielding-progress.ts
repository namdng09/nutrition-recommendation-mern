...
export function registerUnyieldingProgressHandler(): void {
  eventBus.on(
    EVENTS.SCHEDULE_DISH_EATEN,
    async (payload: EventPayloads[typeof EVENTS.SCHEDULE_DISH_EATEN]) => {
      try {
        const { maxStreak } = await getTargetProgress(payload.userId);
        if (maxStreak >= 14) {
          await AchievementService.unlock(
            payload.userId,
            ACHIEVEMENTS.UNYIELDING_PROGRESS.key
          );
        }
      } catch (err) {
        console.error('[Achievement] UNYIELDING_PROGRESS handler error', err);
      }
    }
  );
}
