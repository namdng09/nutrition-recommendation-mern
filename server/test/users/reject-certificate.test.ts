...
import { rejectCertificateRequestSchema } from '~/features/users/user-dto';
...
describe('UserService.rejectCertificate (UC89)', () => {
  afterEach(() => {
    vi.clearAllMocks();
  });

  it('should fail when rejection reason is empty', async () => {
    const result = rejectCertificateRequestSchema.safeParse({
      rejectionReason: ''
    });
  ...
  it('should 404 when user not found', async () => {
    ...
    await expect(
      UserService.rejectCertificate('n1', { rejectionReason: 'Invalid' })
  ...
  it('should 404 when user has no certificate', async () => {
  ...
  it('should throw 400 when certificate already rejected', async () => {
    ...
    await expect(
      ...
      status: 400,
      message: 'Chứng chỉ đã bị từ chối trước đó'
    });
  });

  it('should reject pending certificate and save reason', async () => {
    ...
    const result = await UserService.rejectCertificate('n2', {
      rejectionReason: 'Missing legal proof'
    });

    expect(save).toHaveBeenCalled();
    expect(mockSendMail).toHaveBeenCalled();
    expect(result.certificate.status).toBe(CERTIFICATE_STATUS.REJECTED);
  });
});
