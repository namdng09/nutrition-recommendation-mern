...
export function registerTheDisciplinedHandler(): void {
  for (const event of [
    ...
    eventBus.on(event, async (payload: EventPayloads[typeof event]) => {
      try {
        ...
        if (completedCircleDays >= 20) {
          await AchievementService.unlock(
            payload.userId,
            ACHIEVEMENTS.THE_DISCIPLINED.key
          );
        }
      } catch (err) {
        console.error('[Achievement] THE_DISCIPLINED handler error', err);
      }
    });
  }
}
